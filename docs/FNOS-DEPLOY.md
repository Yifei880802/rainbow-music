...
- `fpk/manifest` 以**注释**留档预期分类取值（音乐类对齐 `Audio,Video Entertainment`），**不注入活跃未知键**——不破坏安装优先于前向声明（本机无 Docker、重装验证推迟到 v0.2.21，无法实机确认未知键行为；且当前解析器已实证忽略该键，活跃声明零收益）；
